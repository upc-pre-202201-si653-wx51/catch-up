<template>
  <v-app theme="light">
    ...
    <v-main>
      <v-container fluid>
        <unavailable-content v-if="errors.count > 0"
                             :errors="errors"/>
        <main-content v-else
...
export default {
  ...
  methods: {
    ...
    // On Source selected behavior
    setSource(source) {
      this.drawer = !this.drawer;
      this.getArticlesForSourceWithLogo(source);
    },

    // Fetch articles for selected Source id
    ...
    // Fetch articles for selected Source with information
